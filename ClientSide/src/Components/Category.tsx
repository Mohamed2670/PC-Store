  ...
  return (
    <>
      <div className="relative inline-block">
        <button
          className="px-2 py-2 text-white bg-blue-500 rounded-lg hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-300"
          onClick={() => setIsDrobDown(!isDrobDown)}
        >
          Categories
        </button>
        {isDrobDown && (
          <ul className="absolute right-0 mt-2 bg-white border border-gray-200 rounded-lg shadow-lg dark:bg-gray-800 z-20">
            {categories.map((cate) => (
              <li
                key={cate.id}
                className="px-4 py-2 text-gray-700 cursor-pointer hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-700"
                onClick={() => {
                  handleSearch(cate.id); // Call the search handler
                  setIsDrobDown(false); // Close the dropdown
                }}
              >
                {cate.name}
              </li>
